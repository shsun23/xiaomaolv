<template>
  <div class="app-container">
    ...
    <!-- 答题窗口 -->
    <el-dialog title="多选题" :visible.sync="QuestionVisible">
      <div
        :rules="rules"
        :model="temp"
        label-position="left"
      ...
      <el-form :model="temp">
        <el-form-item label="选项" prop="usrAns">
          <el-select v-model="temp.usrAns" placeholder="请选择选项" multiple clearable :style="{width: '100%'}">
            <el-option
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 650px; margin-left:0px;">
        ...
        <el-form-item label="选项" prop="rightAns">
          <el-select v-model="temp.rightAns" placeholder="请下拉选择答案选项" multiple clearable :style="{width: '100%'}">
            <el-option
              v-for="(item, index) in ansOptions"
              :key="index"
              :label="item.label"
...
import stu from '@/api/stuResult/stu_res'
...
export default {
  ...
  methods: {
    ...
    // 添加方法
    submitAns() {
      stu.submitAns(this.temp.usrAns).then(response => {
        console.log("ans:" + response.data.ans)
        this.AnsRes = response.data;
      }).catch(error =>{
        console.log(error)
      })
    },
    ...
    getQuestionList() {
      console.log("页面get")
      stu.getQuestionList().then(response => {
